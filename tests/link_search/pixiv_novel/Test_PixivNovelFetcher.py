...
logger = getLogger("media_gathering.link_search.pixiv_novel.PixivNovelFetcher")
...
class TestPixivNovelFetcher(unittest.TestCase):
    ...
    def get_instance(self) -> PixivNovelFetcher:
        with ExitStack() as stack:
            self.mock_login = stack.enter_context(patch("media_gathering.link_search.pixiv_novel.PixivNovelFetcher.PixivNovelFetcher.login"))
    ...
    def test_PixivNovelFetcher(self):
        with ExitStack() as stack:
            mock_login = stack.enter_context(patch("media_gathering.link_search.pixiv_novel.PixivNovelFetcher.PixivNovelFetcher.login"))
    ...
    def test_login(self):
        with ExitStack() as stack:
            mock_aapi = stack.enter_context(patch("media_gathering.link_search.pixiv_novel.PixivNovelFetcher.AppPixivAPI"))
    ...
    def test_fetch(self):
        with ExitStack() as stack:
            mock_save_directory_path = stack.enter_context(patch("media_gathering.link_search.pixiv_novel.PixivNovelFetcher.PixivNovelSaveDirectoryPath.create"))
            mock_downloader = stack.enter_context(patch("media_gathering.link_search.pixiv_novel.PixivNovelFetcher.PixivNovelDownloader"))
