...
logger = getLogger("media_gathering.link_search.LinkSearcher")
...
class TestLinkSearcher(unittest.TestCase):
    ...
    def test_create(self):
        with ExitStack() as stack:
            mock_logger = stack.enter_context(patch.object(logger, "info"))
            mock_notification = stack.enter_context(patch("media_gathering.link_search.LinkSearcher.notification.notify"))
            mock_pixiv_fetcher = stack.enter_context(patch("media_gathering.link_search.LinkSearcher.PixivFetcher"))
            mock_pixiv_novel_fetcher = stack.enter_context(patch("media_gathering.link_search.LinkSearcher.PixivNovelFetcher"))
            mock_nijie_fetcher = stack.enter_context(patch("media_gathering.link_search.LinkSearcher.NijieFetcher"))
            mock_nico_seiga_fetcher = stack.enter_context(patch("media_gathering.link_search.LinkSearcher.NicoSeigaFetcher"))
