...
class TestLikeFetcher(unittest.TestCase):
    ...
    def test_get_like_jsons(self):
        mock_logger_info = self.enterContext(patch("media_gathering.tac.like_fetcher.logger.info"))
        r = MagicMock()

        DUP_NUM = 3
        sample_jsons = list(repeat(self._get_sample_json(), DUP_NUM))
        r.likes.side_effect = lambda user_ids, limit: sample_jsons
        self.fetcher.tac_twitter.scraper = r
        self.fetcher.tac_twitter.target_id = self.target_id
    ...
    def test_fetch(self):
        mock_get_like_jsons = self.enterContext(patch("media_gathering.tac.like_fetcher.LikeFetcher.get_like_jsons"))
        limit = 400
