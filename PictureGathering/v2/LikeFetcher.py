...
class LikeFetcher(V2Base):
    ...
    def _flatten(self, liked_tweet: list[dict]) -> tuple[list[dict], list[dict], list[dict], list[dict]]:
    ...
    def to_convert_TweetInfo(self, fetched_tweets: list[dict]) -> list[TweetInfo]:
        ...
        # liked_tweet のおおまかな構造解析
        # ページに分かれているので平滑化
        data_list, media_list, tweets_list, users_list = self._flatten(liked_tweet)

        # 以下の構造となっている
        # (A)自分のTL上でのlikeツイート
        #    0階層目
        #    = data_list
        #    通常のlikeとRTを直接likeしたものは data_list に含まれる（通常のlike, RTをlike）
        #    引用RTをlikeしたものは data_with_referenced_tweets として扱う（引用RTをlike, data_list に含まれる）
        # (B)RT先ツイート
        #    (A)のRT先のツイート
        #    (A)から見て1階層目
        #    media が含まれているかもしれない
        ...
        # (A)自分のTL上でのlikeツイートを探索
        # data_with_referenced_tweets を取得
        ...
        # (B)RT先ツイートを探索
        # data_with_referenced_tweets の referenced_tweet.id に紐づくツイートを tweets_list 内から探索する
        # 見つかった tweets に attachments があっても、
        # その media_key に対応する media 情報は media_list に含まれてないため、
        # 別途追加で問い合わせを行う必要がある
        ...
        # キーについては互換性がある
        target_data_list = []
        target_data_list.extend(data_list)  # (A)自分のTL上でのlikeツイート
            ...
            match data:
                case {
                    ...
                    for media_key in media_keys:
                        ...
                        if liked_tweet_id not in seen:
                            seen.append(liked_tweet_id)
                case _:
