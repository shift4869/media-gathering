...
logger = getLogger("media_gathering.link_search.PixivNovel.PixivNovelFetcher")
logger.setLevel(WARNING)


class TestPixivNovelFetcher(unittest.TestCase):
    def setUp(self):
        self.TBP = Path("./test/link_search/PixivNovel")

    def get_instance(self) -> PixivNovelFetcher:
        with ExitStack() as stack:
            self.mock_login = stack.enter_context(patch("media_gathering.link_search.PixivNovel.PixivNovelFetcher.PixivNovelFetcher.login"))
    ...
    def test_PixivNovelFetcher(self):
        with ExitStack() as stack:
            mock_login = stack.enter_context(patch("media_gathering.link_search.PixivNovel.PixivNovelFetcher.PixivNovelFetcher.login"))
    ...
    def test_login(self):
        with ExitStack() as stack:
            mock_aapi = stack.enter_context(patch("media_gathering.link_search.PixivNovel.PixivNovelFetcher.AppPixivAPI"))
    ...
    def test_fetch(self):
        with ExitStack() as stack:
            mock_save_directory_path = stack.enter_context(patch("media_gathering.link_search.PixivNovel.PixivNovelFetcher.PixivNovelSaveDirectoryPath.create"))
            mock_downloader = stack.enter_context(patch("media_gathering.link_search.PixivNovel.PixivNovelFetcher.PixivNovelDownloader"))
